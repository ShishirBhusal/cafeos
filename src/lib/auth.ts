...
export interface UserCapabilities {
  ...
  // CafeOS capabilities
  canAccessCafeCounter: boolean
  canAccessCafeKitchen: boolean
  canAccessCafeDashboard: boolean
  canManageCafeMenu: boolean
  canManageCafeStaff: boolean
  canCloseShift: boolean
  canViewCafeReports: boolean
}
...
function mapRolesToCapabilities(roles: string[]): UserCapabilities {
  const capabilities: UserCapabilities = {
    ...
    // CafeOS capabilities
    canAccessCafeCounter: false,
    canAccessCafeKitchen: false,
    canAccessCafeDashboard: false,
    canManageCafeMenu: false,
    canManageCafeStaff: false,
    canCloseShift: false,
    canViewCafeReports: false,
  }
  ...
  roles.forEach(role => {
    switch (role) {
      ...
      case 'vendor':
        ...
        capabilities.canViewProfile = true
        // Cafe owner gets full cafe access
      ...
      case 'waiter':
        capabilities.canAccessCafeCounter = true // Can take orders
        capabilities.canViewProfile = true
        break
...
export const getCurrentUser = cache(async (): Promise<AuthUser | null> => {
  try {
    ...
    const finalCapabilities = isRoleVersionValid ? capabilities : {
      ...
      canViewProfile: true,
      // CafeOS capabilities - restricted
      canAccessCafeCounter: false,
      canAccessCafeKitchen: false,
      canAccessCafeDashboard: false,
      canManageCafeMenu: false,
